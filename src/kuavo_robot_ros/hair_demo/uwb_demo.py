...
class UWBController:
    ...
    def move_to_target(self, target_point, is_exceeding_allow = False):
        """
        控制机器人移动到指定坐标位置附近
        ...
        # 记录当前运动方向及速度
        cur_move_direction = NO_DIRECTION
        next_move_direction = NO_DIRECTION
        vel_x = 0.0
        vel_y = 0.0
        ...
        while not self.is_at_target(target_point, is_exceeding_allow):
            ...
            if self.state == STAND_CODE:
                ...
                # TODO: yaw 角偏移处理待补充
                if abs(self.cur_x - target_point["x"]) > 0.2:
                    # 偏离中心位置，需要左右调整
                    if self.cur_x < target_point["x"]:
                        vel_x = -0.1
                        next_move_direction = RIGHT_DIRECTION
                    else:
                        vel_x = 0.1
                        next_move_direction = LEFT_DIRECTION
                else:
                    if self.cur_y > target_point["y"]:
                        vel_y = -0.1
                        next_move_direction = BACK_DIRECTION
                    else:
                        vel_y = 0.3
                        next_move_direction = FORWARD_DIRECTION
            
            # if cur_move_direction != next_move_direction:
            #     # 移动方向发生变化，添加延时保证稳定性
    ...
    def is_at_target(self, target_point, is_exceeding_allow):
        ...
        if distance_x < 0.3 and distance_y < 0.3:
            return True
        else:
            return False
...
def main():
    ...
    uwb_controller.move_to_target(MIDDLE_POINT_COORDINATE, True)
    print("到达中点，开始执行动作....")
    uwb_controller.execute_action()

    print("stage2: 从中点移动到桌子")
    uwb_controller.move_to_target(TARGET_POINT_COORDINATE)
    print("抵达终点")
